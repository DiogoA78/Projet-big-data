...
browser.get('https://www.uefa.com/uefachampionsleague/statistics/players/distribution/')
time.sleep(2)
...
df = df.replace({'Eric García': "Eric_García"}, regex=True)
df = df.replace({'A. Christensen': "A._Christensen"}, regex=True)
df = df.replace({'Rúben Dias': "Rúben_Dias"}, regex=True)
df = df.replace({'Iñaki Peña': "Iñaki_Peña"}, regex=True)
df = df.replace({'De Ligt': "De_Ligt"}, regex=True)
df = df.replace({'Thiago Silva': "Thiago_Silva"}, regex=True)
df = df.replace({'Juan Jesus': "Juan_Jesus"}, regex=True)


filepath = Path('C:/Users/diogo/OneDrive/Documents/GitHub/Projet-big-data/work/data/LDC/ucl_players_distrib.csv')
