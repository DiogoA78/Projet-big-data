...
browser.get('https://www.uefa.com/uefaeuropaleague/statistics/players/disciplinary/')
time.sleep(2)
...
df = df.replace({'Miguel Crespo': "Miguel_Crespo"}, regex=True)
df = df.replace({'Orkun Kökçü': "Orkun_Kökçü"}, regex=True)
df = df.replace({'Richard Almeida': "Richard_Almeida"}, regex=True)



filepath = Path('C:/Users/diogo/OneDrive/Documents/GitHub/Projet-big-data/work/data/EL/el_stats_players_discip.csv')
